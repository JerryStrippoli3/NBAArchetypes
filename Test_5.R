...
k6 <- kmeans(master_table_std %>% select(-Player, -Team), centers = 6, nstart = 25)
k7 <- kmeans(master_table_std %>% select(-Player, -Team), centers = 7, nstart = 25)
...
p5 <- fviz_cluster(k6, geom = "point",  data = master_table_std %>% select(-Player, -Team)) + ggtitle("k = 6")
p6 <- fviz_cluster(k7, geom = "point",  data = master_table_std %>% select(-Player, -Team)) + ggtitle("k = 7")
